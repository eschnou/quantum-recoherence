...
\begin{itemize}
  \item \textbf{Small system.} We test $D = 128$ to $D = 1024$ (7--10 qubits).
    ...
    prediction~\citep{Strasberg2024}, but the accessible range spans less than one
...
generality of the filtering pattern reported in Section~\ref{sec:freq-results}:
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{../results/scaling_profiles.png}
...
(Figure~\ref{fig:scaling-gap}). The dominant factor is the projector asymmetry
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{../results/scaling_vs_D.png}
...
\begin{table}[H]
  ...
  \begin{tabular}{lllcc}
    ...
    Left-block $\geq 2/5$ & Spatial subsystem & 0.812 & 0.083 & 4.4$\times$ \\
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{../results/projectors.png}
...
The branch-level analysis in Section~\ref{sec:branches} uses the operational
threshold $\varepsilon < 0.3$. Figure~\ref{fig:threshold} shows how the
results depend on this choice.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{../results/threshold_detail.png}
