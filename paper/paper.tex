...
A non-scrambling control---replacing the Ising
Hamiltonian with independent single-qubit rotations at matched $p_1$---produces
no stable filtering, demonstrating that projector asymmetry alone is insufficient
and that scrambling dynamics are a necessary ingredient.
We further show that the filtering operates simultaneously at multiple
\emph{spatial} scales: splitting 8~qubits into two groups and applying
independent projectors to each yields Born filtering at both the
single-group level ($p_1 = 0.69$, $2\times$ gap) and the coarser
``at least one group active'' level ($p_1 = 0.90$, $9\times$ gap), with the
coarser observer perceiving their Born probability as fundamental.
The effect is robust across system sizes ($D = 128$ to $1024$), coarse-graining
projectors, and decoherence thresholds.
\end{abstract}
...
frequency---though the Ising curve decays more gradually and its minimum is
shifted slightly toward $p_1$, as one might expect from a structured Hamiltonian
compared to a featureless random matrix.
...
The filtering strength scales with the projector asymmetry $|p_1 - 1/2|$
(Table~\ref{tab:projectors}), and vanishes when $p_1 = 0.5$. This raises a
natural question: is the filtering a nontrivial dynamical effect, or merely a
kinematic consequence of having unequal projector subspaces? In other words,
what is the null hypothesis?

We test this by replacing the Ising Hamiltonian with a product rotation
$U_{\text{prod}} = e^{-i\theta X}{}^{\otimes m}$, where each qubit rotates
independently (with $\theta = h_x \Delta t$ to match the rotation strength of
the Ising model). This unitary generates no entanglement and no scrambling. The
projectors, initial states, $p_1$, and analysis pipeline are identical to the
Ising experiment.

Figure~\ref{fig:product-control} shows the result. Under the Ising (scrambling)
dynamics, the Born frequency class is strongly decoherent
($\varepsilon_{\text{Born}} = 0.087$) with a $3.1\times$ gap over the
combinatorial peak. Under the product rotation, the gap collapses to
$1.3\times$, and \emph{no frequency class achieves low $\varepsilon$}: even the
Born frequency has $\varepsilon \approx 0.5$, indicating substantial recoherence
everywhere. Sweeping $L$ from 5 to 25 confirms that this is not a matter of
convergence: the product rotation produces erratic oscillations in both
$\varepsilon_{\text{Born}}$ and $\varepsilon_{\text{comb}}$ with no stable
filtering at any history length, consistent with the quasiperiodic dynamics of
non-interacting qubits.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{../results/product_control.png}
  \caption{Non-scrambling control. Recoherence parameter $\varepsilon(n_1/L)$
    for the Ising chain (red) and a product rotation (gray) at $D = 512$,
    $L = 15$, $p_1 = 0.746$. Averaged over 10 Haar-random initial states;
    shading shows $\pm 1\sigma$. The product rotation shows no filtering: all
    frequency classes are substantially recoherent
    ($\varepsilon \gtrsim 0.5$).}
  \label{fig:product-control}
\end{figure}

The projector asymmetry determines \emph{which} frequencies can survive as
decoherent branches, but it does not by itself cause any frequency class to
decohere. The scrambling dynamics are a necessary second ingredient: they spread
amplitude across the Hilbert space in a way that allows the finite-dimensional
geometry to selectively preserve Born-typical branches.
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{../results/branch_survivors.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{../results/branch_gram.png}
...
each group at every time step.  The four joint outcomes $(00, 01, 10, 11)$
partition $\mathcal{H}$ into orthogonal subspaces.  From these we construct two
scales of binary projectors:
\begin{itemize}
...
We track two nesting metrics across $L$ (averaged over 5--20 seeds,
$\varepsilon < 0.3$):
...
\noindent S1/S2: mean decoherent branches. \emph{Containment}: fraction of
scale-1 survivors inside a surviving scale-2 branch.
\emph{Emergence}: fraction of scale-2 survivors with \emph{all}
scale-1 constituents dead. \emph{Leak}: fraction of dead scale-2
branches containing any scale-1 survivor.
...
\begin{itemize}
  ...
  \item \textbf{Single Hamiltonian family.} We test only the transverse-field Ising
    chain. Born-rule filtering may depend on properties of the dynamics (e.g.,
    integrability vs.\ chaos) that we have not varied. The Ising Hamiltonian is
    integrable, unlike the chaotic random matrix models used by Strasberg, so the
    agreement is encouraging but does not establish universality.
    The product-rotation control (Sec.~\ref{sec:product-control}) shows that
    \emph{some} dynamical complexity is necessary, but the boundary between
    sufficient and insufficient scrambling remains unexplored.
...
A non-scrambling control (product rotation at matched $p_1$) produces no stable
filtering, showing that projector asymmetry is necessary but not sufficient:
the dynamics must spread amplitude across the Hilbert space for
finite-dimensional geometry to selectively preserve Born-typical branches.
